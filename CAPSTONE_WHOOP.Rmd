...
ggplot(whoop_df_explore, aes(x = maxHR, y = strain)) + 
  ...
  labs(title="Scatterplot of Max Heart Rate and Strain Score", x="Max Heart Rate", y = "")
...
# K-fold CV
strain_mse_10foldcv = NULL

for(i in 1:10) {
  model = glm(strain_model_formula, data = whoop_df_strain_reg)
  strain_mse_10foldcv[i] = cv.glm(whoop_df_strain_reg, model, K = 10)$delta[1]
}

# mean of RMSE of 10-fold cross-validation
sqrt(strain_mse_10foldcv)
mean(sqrt(strain_mse_10foldcv))
...
# K-fold CV
sleep_mse_10foldcv = NULL

for(i in 1:10) {
  model = glm(sleep_model_formula, data = whoop_df_sleepPerform_reg)
  sleep_mse_10foldcv[i] = cv.glm(whoop_df_sleepPerform_reg, model, K = 10)$delta[1]
}
