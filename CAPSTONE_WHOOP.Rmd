...
```{r}
# using glm function to create first model
strain_model1 = glm(strain ~ maxHR + averHR + cal, data = whoop_df_explore)
summary(strain_model1)

strain_model2 = glm(strain ~ maxHR + cal, data = whoop_df_explore)
...
for(i in 1:10) {
  model = glm(strain_model_formula, data = whoop_df_strain_reg)
...
sleepPerform_model1 = glm(sleepPerform ~ timeLightSleep + timeREMSleep + timeDeepSleep + sleepCycles, data = whoop_df_explore)
summary(sleepPerform_model1)

sleepPerform_model2 = glm(sleepPerform ~ timeLightSleep + timeREMSleep + timeDeepSleep, data = whoop_df_explore)
summary(sleepPerform_model2)

# linear regress to predict sleepPerform using total time spent in bed and sleeping
sleepPerform_model3 = glm(sleepPerform ~ timeInBed + totalSleep + sleepCycles, data = whoop_df_explore)
summary(sleepPerform_model3)

sleepPerform_model4 = glm(sleepPerform ~ timeInBed + totalSleep, data = whoop_df_explore)
...
sleep_model_formula = as.formula(sleepPerform ~ timeInBed + totalSleep)
sleep_model_formula

# additional formulas for potential models of sleep performance
sleep_model_formula2 = as.formula(sleepPerform ~ timeLightSleep + timeREMSleep + timeDeepSleep + sleepCycles)
sleep_model_formula3 = as.formula(sleepPerform ~ timeLightSleep + timeREMSleep + timeDeepSleep)
sleep_model_formula4 = as.formula(sleepPerform ~ timeInBed + totalSleep + sleepCycles)
...
# mean of RMSE of 10-fold cross-validation
sqrt(sleep_mse_10foldcv)
mean(sqrt(sleep_mse_10foldcv))
...
recovery_model1 = glm(recovery ~ hrv + sleepPerform, data = whoop_df_explore)
summary(recovery_model1)

recovery_model2 = glm(recovery ~ hrv + timeInBed + totalSleep, data = whoop_df_explore)
...
whoop_df_recovery_reg$recoveryPredict = predict(recovery_model1, whoop_df_recovery_reg)
...
new_recovery$prediction = predict(recovery_model1, newdata = new_recovery)
new_recovery
...
recovery_model_formula = as.formula(recovery ~ hrv + sleepPerform)
recovery_model_formula

# additional formulas for potential models of sleep performance
recovery_model_formula2 = as.formula(recovery ~ hrv + timeInBed + totalSleep)
recovery_model_formula3 = as.formula(recovery ~ hrv + totalSleep)
recovery_model_formula4 = as.formula(recovery ~ hrv + restHR + totalSleep)

# K-fold CV
recovery_mse_10foldcv = NULL
...
# predict from a full model
whoop_df_recovery_reg$recoveryPredict.cv = predict(recovery_model1, data = whoop_df_recovery_reg)
